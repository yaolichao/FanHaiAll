...
namespace Astronergy.MES.IVTest.BLL
{
    /// <summary>
    /// 字符串扩展类。
    /// </summary>
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    catch (Exception ex)
                    {
                        dbTrans.Rollback();
                        bAllSuccess = false;
                        EventLog.WriteEntry("Astronergy.MES.IVTest", string.Format("{0}-{1}-{2}-{3}", deviceName, lotNum, testTime, ex.Message), EventLogEntryType.Error);
                        continue;
                    }
                    finally
                    {
