...
namespace Astronergy.MES.Report.DataAccess
{
    ...
    /// 批次数据访问类
    /// </summary>
    public class OEMLotDataForAllAccess : BaseDBAccess
    {
        /// <summary>
        /// 获取指定日期区间的批次清单数据。
        /// </summary>
        ...
        public DataSet GetLotListData(
                            ...
                            string user)
        {
            const string storeProcedureName = "SP_QRY_OEM_IDTOPROCEDURE";
            using (DbConnection con = this._db.CreateConnection())
            {
                ...
                this._db.AddInParameter(cmd, "p_user", DbType.String, user);
        ...
        public DataSet GetLotDetail(string lotKey,int type , string user)
        {
            const string storeProcedureName = "SP_QRY_OEM_IDTOPROCEDURE_DETIAL";
            using (DbConnection con = this._db.CreateConnection())
            {
                DbCommand cmd = con.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
