...
namespace Astronergy.MES.Report.DataAccess
{
    ...
    /// 批次数据访问类
    /// </summary>
    public class OEMLotData4SkyPowerAccess : BaseDBAccess
    {
        /// <summary>
        /// 获取指定日期区间的批次清单数据。
        /// </summary>
        ...
        public DataSet GetLotListData(
                            ...
                            string partNumber)
        {
            const string storeProcedureName = "SP_QRY_OEM_LOTDATA4SKYPOWER";
        ...
        {
            const string storeProcedureName = "SP_QRY_OEM_LOTDATADETAIL4SKYPOWER";
            using (DbConnection con = this._db.CreateConnection())
            {
                DbCommand cmd = con.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
