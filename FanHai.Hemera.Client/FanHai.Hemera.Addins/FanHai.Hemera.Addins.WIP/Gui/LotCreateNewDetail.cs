...
namespace FanHai.Hemera.Addins.WIP
{
    ...
    /// 表示创建批次的控件类。
    /// </summary>
    public partial class LotCreateNewDetail : BaseUserCtrl
    {
        LotCreateEntity _entity;

        LineSettingEntity _lineSettingEntity = new LineSettingEntity();

        LotCreateNewDetailModel _model;                    //批次创建的参数数据。
        bool _isBatch = false;
        /// <summary>
        /// 是否显示选择设备的对话框。
        /// </summary>
        ...
        public LotCreateNewDetail(LotCreateNewDetailModel model, bool isBatch, IViewContent view)
        {
            InitializeComponent();
        ...
        {            
            ...
            BindOperations();
            BindLine();
            if (string.IsNullOrEmpty(_model.OrderKey))
            {
                MessageService.ShowMessage("工单号为空请返回创批界面选择工单后重新创批！", "提示");
                return;
            }
        }
        /// <summary>
        ...
        private void BindLotList()
        {
            ...
            if (!string.IsNullOrEmpty(_model.OrderKey))
            {
                dsLotNum = _entity.CreateLotNumByRule(_model.OrderKey,_model.Count);     //传入工单主键创建除流水号之外的批次信息
                 if (!string.IsNullOrEmpty(_entity.ErrorMsg))
                {
                    MessageBox.Show(_entity.ErrorMsg, "系统提示");
                    flag = 0;
                }
                else
                    flag = 1;
            ...
            for (int i = 0; i < this._model.Count; i++)
            {
                ...
                dr["CELLPOWER"] = 0;
                dr["INDEX"] = i + 1;
                dr["LOT_NUMBER"] = flag == 0 ? "" : dsLotNum.Tables[0].Rows[i]["LOT_NUMBER"].ToString();
        ...
        private void InitControlValue()
        {
            ...
            this.lblMenu.Text = this._view.TitleName;
            ...
            this.gvNewLot.FocusedColumn = this.CELLPN1;
        ...
        private void btnClose_Click(object sender, EventArgs e)
        {
            ...
            //重新打开批次创建视图。
            LotCreateNewViewContent view = new LotCreateNewViewContent(this._model, this._isBatch);
        ...
        private void tsbOK_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.lueEquipment.Text))
            {
                MessageService.ShowMessage("请选择要创建的单串焊设备！", "提示");
            ...
            if (_isCheckLotLine)
            {
                ...
                if (!lotLineKey.Equals(this.lueLine.EditValue.ToString()))
                {
                    MessageService.ShowMessage("创建的线别和设备带出的实际线别不符！", "提示");
                    this.lueLotLineInfo.Select();
                    return;
                }
            }
            ...
            htMaindata.Add("OPERATION_NAME", operations);
            htMaindata.Add("EQUIPMENT_KEY", equipmentkey);
            htMaindata.Add("LINE_KEY", lineKey);
            ...
            DataSet dsReturn = _entity.CeateNewLot(dsParams);
        ...
        {
            string strFactoryRoomKey = _model.RoomKey; //this.lueFactoryRoom.EditValue.ToString();
        ...
        private void BindEquipment()
        {
            ...
            string strFactoryRoomName = _model.RoomName;//this.lueFactoryRoom.Text;
            string strFactoryRoomKey = _model.RoomKey == null ? string.Empty : _model.RoomKey;
            string strLines = PropertyService.Get(PROPERTY_FIELDS.LINES);
            //如果工厂车间或者工序或者线别主键为空。
