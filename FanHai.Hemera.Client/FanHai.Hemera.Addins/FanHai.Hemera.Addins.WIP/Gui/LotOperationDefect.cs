...
{
    /// <summary>
    /// 表示批次操作（电池片/组件不良）明细的控件类。
    /// </summary>
    public partial class LotOperationDefect : BaseUserCtrl
    {
        ...
        /// <param name="e"></param>
        private void LotOperationDefect_Load(object sender, EventArgs e)
        {
            this.lblMenu.Text = "质量管理>不良品处理>不良判定";
        ...
        private void BindReasonCodeClass(string categoryKey)
        {
            ...
            //电池片不良,只显示电池片分类。
            if (this._model.OperationType == LotOperationType.CellDefect)
            {
        ...
        {
            DataTable dtList = CommonUtils.CreateDataTable(new WIP_DEFECT_FIELDS());
        ...
        private void ResetControlValue()
        {
            ...
            dr[WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY] = 0;
            //电池片不良。
            if (this._model.OperationType == LotOperationType.CellDefect)
            {
                this._activity = ACTIVITY_FIELD_VALUES.FIELD_ACTIVITY_CELLDEFECT;
            }
            else
            {
                this._activity = ACTIVITY_FIELD_VALUES.FIELD_ACTIVITY_DEFECT;
            }
            dtList.Rows.Add(dr);
        ...
        private void gvList_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            ...
            {
                dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_KEY] = string.Empty;
                dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_NAME] = string.Empty;
                if (this._model.OperationType == LotOperationType.CellDefect)
                {
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY] = 0;
                }
                if (e.Column == this.gclStep)
                {
                    string stepKey = Convert.ToString(dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_STEP_KEY]);
                    ...
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_STEP_NAME] = stepName;
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_ENTERPRISE_KEY] = enterpriseKey;
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_ENTERPRISE_NAME] = enterprisenName;
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_ROUTE_KEY] = routeKey;
                    dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_ROUTE_NAME] = routeName;
                }
            }
            else if (e.Column == this.gclReasonCode)
            {
                ...
                string codeKey = Convert.ToString(dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_KEY]);
                ...
                {
                    int count = dtList.AsEnumerable().Count(dr => Convert.ToString(dr[WIP_DEFECT_FIELDS.FIELD_REASON_CODE_KEY]) == codeKey
                                                                  && Convert.ToString(dr[WIP_DEFECT_FIELDS.FIELD_STEP_KEY]) == stepKey);
                    //原因代码在列表中存在。
                    if (count > 1)
                    {
                ...
                dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_NAME] = codeName;
            }
            //数量列 且 原因分类是电池片。
                    ...
                    && BASEDATA_CATEGORY_NAME.Basic_ClassOfRCodeValue_Cell.IndexOf(string.Format("'{0}'", reasonCodeClass)) >= 0)
            {
                ...
                double scrapQty = dtList.AsEnumerable().Sum(dr => Convert.ToDouble(dr[WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY]));
                double leftQty = qty - scrapQty;
                if (leftQty < 0)
                {
                    MessageService.ShowMessage("电池片不良数量不能超过当前电池片数量。");
                    if (this._model.OperationType == LotOperationType.CellDefect)
                    {
                        dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY] = 0;
                    }
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageService.ShowMessage("不良原因列表中必须至少有一条记录。", "提示");
                return;
            }
        ...
        private void gvList_CustomDrawCell(object sender, RowCellCustomDrawEventArgs e)
        {
            ...
            {
                DataTable dtList = this.gcList.DataSource as DataTable;
                e.DisplayText = Convert.ToString(dtList.Rows[e.RowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_NAME]);
            }
        ...
        private void tsbOK_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageService.ShowMessage("不良原因列表中至少必须有一条记录。", "提示");
                return;
            }
            //问题工序必须全部输入
            List<DataRow> lst = (from item in dtList.AsEnumerable()
                                 where string.IsNullOrEmpty(Convert.ToString(item[WIP_DEFECT_FIELDS.FIELD_STEP_KEY]))
                                 select item).ToList<DataRow>();
            if (lst.Count() > 0)
            {
                MessageService.ShowMessage("不良原因列表中的【问题工序】必须输入。", "提示");
            ...
            lst=(from item in dtList.AsEnumerable()
               where string.IsNullOrEmpty(Convert.ToString(item[WIP_DEFECT_FIELDS.FIELD_REASON_CODE_KEY])) 
               select item).ToList<DataRow>();
            if (lst.Count() > 0)
            {
                MessageService.ShowMessage("不良原因列表中的【原因名称】必须输入。", "提示");
                this.gvList.FocusedColumn = this.gclReasonCode;
                this.gvList.FocusedRowHandle = dtList.Rows.IndexOf(lst[0]);
                this.gvList.ShowEditor();
                return;
            ...
            //不良原因中的数量必须输入值且大于0
            lst = (from item in dtList.AsEnumerable()
                   where string.IsNullOrEmpty(Convert.ToString(item[WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY]).Trim())
                       || Convert.ToInt32(item[WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY]) <= 0
                   select item).ToList<DataRow>();
            if (lst.Count() > 0)
            {
            ...
            double qty=Convert.ToDouble(this.teQty.Text);
            //电池片不良数量总和不能超过当前电池片数量
            double defectQty = dtList.AsEnumerable()
                                    .Where(dr => BASEDATA_CATEGORY_NAME.Basic_ClassOfRCodeValue_Cell.IndexOf(string.Format("'{0}'", Convert.ToString(dr[WIP_DEFECT_FIELDS.FIELD_REASON_CODE_CLASS]))) >= 0)
                                    .Sum(dr => Convert.ToDouble(dr[WIP_DEFECT_FIELDS.FIELD_DEFECT_QUANTITY]));
            double leftQty = qty - defectQty;
            if (leftQty < 0)
            {
                MessageService.ShowMessage("电池片不良数量不能超过当前电池片数量。", "提示");
                return;
            }
            //组件不良操作 电池片数量都不变
            if (this._model.OperationType == LotOperationType.Defect)
            {
                leftQty = qty;
            }
            ...
            //组织不良原因数据
            DataTable dtDefect = dtList.Copy();
            dtDefect.TableName = WIP_DEFECT_FIELDS.DATABASE_TABLE_NAME;         
            foreach (DataRow dr in dtDefect.Rows)
            {
                dr[WIP_DEFECT_FIELDS.FIELD_EDITOR] = this._model.UserName;
                dr[WIP_DEFECT_FIELDS.FIELD_EDIT_TIME] = DBNull.Value;
                dr[WIP_DEFECT_FIELDS.FIELD_EDIT_TIMEZONE] = timezone;
            }
            ...
            //执行不良。
            this._entity.LotDefect(dsParams);
            if (!string.IsNullOrEmpty(this._entity.ErrorMsg))
            {
                MessageService.ShowError(this._entity.ErrorMsg);
            }
        ...
        private void gvList_ShowingEditor(object sender, CancelEventArgs e)
        {
            ...
            {
                string stepKey = Convert.ToString(dtList.Rows[this.gvList.FocusedRowHandle][WIP_DEFECT_FIELDS.FIELD_STEP_KEY]);
                if (!string.IsNullOrEmpty(stepKey))
                {
                    int rowIndex = this.rilueStep.GetDataSourceRowIndex(POR_ROUTE_STEP_FIELDS.FIELD_ROUTE_STEP_KEY, stepKey);
                    string categoryKey = Convert.ToString(this.rilueStep.GetDataSourceValue(POR_ROUTE_STEP_FIELDS.FIELD_DEFECT_REASON_CODE_CATEGORY_KEY, rowIndex));
            ...
            {
                string stepKey = Convert.ToString(dtList.Rows[this.gvList.FocusedRowHandle][WIP_DEFECT_FIELDS.FIELD_STEP_KEY]);
                string codeClass = Convert.ToString(dtList.Rows[this.gvList.FocusedRowHandle][WIP_DEFECT_FIELDS.FIELD_REASON_CODE_CLASS]);
