...
namespace FanHai.Hemera.Addins.MM
{
    ...
    /// 表示领料数据的窗体类。
    /// </summary>
    public partial class MaterialRequisitionCtrl : BaseUserCtrl
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        {
            layoutControlItem6.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0009}");//领料单号
            smbSelect.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0010}");//查询
            lciMaterialLot.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0009}");//领料单号
            layoutControlItem9.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0012}");//状态
            lciFactoryRoom.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0013}");//领料车间
            lciOperation.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0014}");//工序名称
            lciStoreName.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0016}");//线上仓名称
            lciWorkorderNo.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0017}");//工单号
            layoutControlItem8.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0018}");//创建人
            layoutControlItem11.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0019}");//创建时间
            lciMaterialCode.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0020}");//原材料编码
            lciMaterialDescription.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0021}");//原材料描述
            lciSupplierName.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0022}");//原材料供应商
            lciIssueQty.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0023}");//领料数量
            layoutControlItem10.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0024}");//单位
            lciMemo.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0025}");//备注
            sbtRowAdd.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0026}");//新增
            sbtRowDel.Text = StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.lbl.0027}");//移除
        ...
        private void sbtRowAdd_Click(object sender, EventArgs e)
        {
            ...
            if (_status == "New")
            {
                if (Convert.ToInt32(dsCheckNum.Tables[0].Rows[0]["COUNT"].ToString()) > 0)
                {
                    ...
                    MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0001}"), StringParser.Parse("${res:Global.SystemInfo}"));
                    return;
                }
            }
            ...
            if (dtSource != null)
            {
                DataRow[] drs = dtSource.Select("AUFNR='" + lueWorkOrderNo.Text.Trim() + "'AND MATNR= '" + lueMaterialCode.Text.Trim()+"'");
                if (drs.Length >= 1)
                {
                    //MessageBox.Show("数据表中存在重复数据！", "系统提示");
                    MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0002}"), StringParser.Parse("${res:Global.SystemInfo}"));
                    return;
                }
            }
            else
        ...
        private bool CheckParameter()   
        {
            ...
            {
                //MessageBox.Show("领料单号不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0003}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueFactoryRoom.Text.Trim()))
            {
                //MessageBox.Show("车间不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0004}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueOperation.Text.Trim()))
            {
                //MessageBox.Show("工序不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0005}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueStoreName.Text.Trim()))
            {
                //MessageBox.Show("线上仓不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0006}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueWorkOrderNo.Text.Trim()))
            {
                //MessageBox.Show("工单不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0007}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueMaterialCode.Text.Trim()))
            {
                //MessageBox.Show("物料不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0008}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(lueSupplierName.Text.Trim()))
            {
                //MessageBox.Show("供应商不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0009}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            if (string.IsNullOrEmpty(txtIssueQty.Text.Trim()))
            {
                //MessageBox.Show("数量不能为空！", "系统提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0010}"), StringParser.Parse("${res:Global.SystemInfo}"));
                return false;
            }
            return true;
        }
        ...
        private void sbtRowDel_Click(object sender, EventArgs e)
        {
            if (this.gvInf.GetFocusedRow() != null)
            {
                ...
                {
                    int counter = 0;
                    for (int i = 1; i < dtCheck.Rows.Count + 1; i++)
                    {
                        if (dtCheck.Rows[i - 1].RowState != DataRowState.Deleted && dtCheck.Rows[i - 1].RowState != DataRowState.Detached)
                        {
                            counter++;
                        }
                    }
                    if (counter == 1)
                    {
                        ...
                        MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0011}"), StringParser.Parse("${res:Global.SystemInfo}"));
                        return;
                    }
                }
            ...
            else
            {
                ...
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0012}"), StringParser.Parse("${res:Global.SystemInfo}"));
            }
        }
        /// <summary>修改操作
        /// 修改操作
        ...
        private void tspEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                //MessageBox.Show("当前领料单状态为'" + txtStatus.Text + "'！不能修改", "系统错误提示");
                MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0013}") 
                    + txtStatus.Text
                    + StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0014}"), StringParser.Parse("${res:Global.SystemInfo}"));
            }
        }
        ...
        {
            MaterialReqOrReturnEntity materialReqOrReturnEntity = new MaterialReqOrReturnEntity();
            //if (MessageBox.Show(StringParser.Parse("是否对当前领料单进行删除？"),
            if (MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0029}"),
                   StringParser.Parse("${res:Global.SystemInfo}"), MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                if (CheckNumTrueOrFalse(txtNum.Text.Trim()) == false)
                {
                ...
                if (txtStatus.Text == "已创建")
                {
                    DataSet dsReturn = materialReqOrReturnEntity.DeleteNum(txtNum.Text.Trim(), PropertyService.Get(PROPERTY_FIELDS.USER_NAME_MZ));
                    if (string.IsNullOrEmpty(materialReqOrReturnEntity.ErrorMsg))
                    {
                        //领料单  已成功删除！
                        MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0024}") 
                            + txtNum.Text.Trim()
                            + StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0030}"), StringParser.Parse("${res:Global.SystemInfo}"));
                        _status = "New";
                        Status(_status);
                    }
                    else
                ...
                {
                    //MessageBox.Show("当前领料单状态为'" + txtStatus.Text + "'！不能删除", StringParser.Parse("${res:Global.SystemInfo}"));
                    MessageBox.Show(StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0031}") 
                        + txtStatus.Text
                        + StringParser.Parse("${res:FanHai.Hemera.Addins.MaterialRequisitionCtrl.msg.0032}"), StringParser.Parse("${res:Global.SystemInfo}"));
                }
            }
        }
