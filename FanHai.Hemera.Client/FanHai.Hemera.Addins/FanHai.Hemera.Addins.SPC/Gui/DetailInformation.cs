...
{
    public partial class DetailInformation : BaseDialog
    {
        private string _pointkey = null;
        SpcEntity spcEntity = new SpcEntity();
        string _edcInsKey = string.Empty;
        public DetailInformation():base("明细信息")
        {
            InitializeComponent();
        }

        public DetailInformation(string pointkey)
            : base("明细信息")
        {
        ...
        private void PointInformation_Load(object sender, EventArgs e)
        {
            ...
            DataTable dtInformation = spcEntity.GetPointDetailInformation(_pointkey).Tables[SPC_PARAM_DATA_FIELDS.DB_FOR_POINTS];
