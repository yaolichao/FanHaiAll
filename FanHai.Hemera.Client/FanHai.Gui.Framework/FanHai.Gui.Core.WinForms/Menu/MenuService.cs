...
{
    internal sealed class MenuUtils
    {
    ...
    public static class MenuService
    {
        ...
        public static object CreateMenuItemFromDescriptor(MenuItemDescriptor descriptor)
        {
            Codon codon = descriptor.Codon;
            string type = codon.Properties.Contains("type") ? codon.Properties["type"] : "Command";
            bool createCommand = codon.Properties["loadclasslazy"] == "false";
            bool displayMenu = IsDisplayMenu(descriptor.Codon);
            if (displayMenu == false) return null;
            switch (type)
            {
                case "Separator":
        ...
        internal static ArrayList ConvertSubItems(IList items)
        {
            ...
            if (items != null)
            {
                ...
                {
                    object obj = CreateMenuItemFromDescriptor(descriptor);
                    if (obj != null)
                    {
                        r.Add(obj);
                    }
        ...
        private static bool bDisplaySeparator = false;
        /// <summary>
        /// 是否显示菜单。
        /// </summary>
        /// <returns>true:显示菜单。false:不显示菜单。</returns>
        public static bool IsDisplayMenu(Codon codon)
        {
            if (codon == null) return true;
            string type = codon.Properties.Contains("type") ? codon.Properties["type"] : "Command";
            //根据用户权限判断是否显示菜单。
            ...
            if (type == "Separator")
            {
                bool visible = bDisplaySeparator;
                if (bDisplaySeparator) bDisplaySeparator = false;
                return visible;
            }
            else if (string.IsNullOrEmpty(codon.PrivilegeId))
            {
                return false;
            }
            else
            {
                if (privilegeStr.IndexOf(codon.PrivilegeId + ",") != -1 || codon.PrivilegeId == "9999999")
                {
                    bDisplaySeparator = true;
                    return true;
                }
                else
                {
